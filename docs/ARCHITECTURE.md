...
| Condition | Behaviour |
|---|---|
| Artifact absent | warn, degrade to CountSketch |
| `ALS_EXPECTED_SHA256` set and mismatched | **refuse to start** (integrity failure) |
| Invalid, `ALS_REQUIRE_VALID_ARTIFACT=true` | **refuse to start** |
| Invalid, default settings | log error, degrade to CountSketch |

Nothing trains or rebuilds at startup.
