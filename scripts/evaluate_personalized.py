...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run leakage-safe, per-user held-out evaluation for popularity, "
            "the existing CountSketch CF model, and the current hybrid."
        )
    )
...
def _default_paths(args: argparse.Namespace) -> tuple[Path, Path, Path]:
    ...
    scope = "full" if args.max_evaluation_users == 0 else f"sample_{args.max_evaluation_users}"
